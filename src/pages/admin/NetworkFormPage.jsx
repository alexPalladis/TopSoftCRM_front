...
export default function NetworkFormPage() {
    ...
    networksApi
      ...
      .then((res) => {
        ...
        setForm({
          afm: n.afm || "",
          eponymia: n.eponymia || "",
          nomimosEkprosopos: n.nomimosEkprosopos || "",
          epaggelma: n.epaggelma || "",
          doy: n.doy || "",
          address: n.address || "",
          city: n.city || "",
          tk: n.tk || "",
          phoneFixed: n.phoneFixed || "",
  ...
  const handleCommissionChange = (i, field, value) => {
    setCommissions((prev) =>
      prev.map((c, idx) => {
        ...
        // Αν αλλάξει το %, υπολόγισε αυτόματα τιμή πώλησης
        if (field === "percentage" && value !== "") {
          const base = PRODUCTS[i].defaultPrice;
          updated.salePrice = (base * (1 - parseFloat(value) / 100)).toFixed(2);
        }
        return updated;
      }),
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = { ...form };
      if (isEdit) {
        await networksApi.update(id, payload);
      } else {
        await networksApi.create(payload);
      }
      setSubmitted(true);
      setTimeout(() => navigate("/networks"), 1500);
    } catch (err) {
      ...
      if (
        ...
        setErrors({ afm: "Υπάρχει ήδη network με αυτό το ΑΦΜ" });
      } else {
        setGlobalError(msg);
      }
    } finally {
    ...
    <Box sx={{ maxWidth: 900 }}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          ...
          <Box>
            ...
            >
              {isEdit ? "Διόρθωση Network" : "Νέο Network"}
            </Typography>
            <Typography sx={{ fontSize: 13, color: "#9ca3af" }}>
              {isEdit ? `ID: ${id}` : "Καταχώρηση νέου network"}
            </Typography>
          </Box>
        </Box>
      ...
      {submitted && (
        <Alert severity="success" sx={{ mb: 2 }}>
          Το network {isEdit ? "ενημερώθηκε" : "καταχωρήθηκε"} επιτυχώς!
        </Alert>
      )}
      {globalError && (
      ...
      <Stack spacing={2}>
        ...
        >
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={4}>
              <Field label="Username" req error={errors.username}>
                <TextField
                  fullWidth
                  size="small"
                  value={form.username}
            ...
            <Grid item xs={12} md={4}>
              <Field
                label={
                  isEdit
                    ? "Νέο password (αφήστε κενό για να μην αλλάξει)"
                    : "Password"
                }
                req={!isEdit}
